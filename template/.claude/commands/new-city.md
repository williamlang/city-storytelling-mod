...
`Read` the file `carto/processed/map.svg`. This is a single SVG generated by the mod that overlays everything from step 2 in one image: terrain in hypsometric tints (green lowland → tan → brown → white peaks), water bodies in depth-shaded blue, district outlines, named and unnamed roads, intersections, and labels. The data is the same data that backed the text chunks; the image is what's useful when shape and adjacency matter more than the numbers.
