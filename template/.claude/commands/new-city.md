...
- **Integrations** — the list of enabled peer-mod integrations, written to `settings.json.integrations[]` (the authoritative per-city allowlist — see CLAUDE.md "Peer-mod integration gate"). **Config mode:** use the `integrations` line verbatim — the wizard already resolved detection + the player's opt-outs (a missing/blank line means none enabled). **Chat mode:** default is opt-*out*, not opt-in — read `snapshot.mods.loaded[]` and enable every supported integration that's detected, then let the player drop any they don't want. The supported ids and their detection cues:
  - `"elections"` — a `mods.loaded[]` entry whose `name`/`id` starts with `Elections`. *"Elections is loaded — I'll fold the mayoral races into the story; say so if you'd rather I leave politics soft."*
  - `"infoloom"` — an entry whose `name`/`id` starts with `InfoLoom`. *"InfoLoom is loaded — I'll ground the economy and demographics on its real numbers."*
  - `"customchirps"` — an entry whose `name`/`id` is `Custom Chirps` / `CustomChirps`. *"Custom Chirps is loaded — new events will surface as short chirps in your in-game feed; say so if you'd rather keep the story to this panel."*

  Include each detected one by default and mention it in a single line as above; omit any whose mod isn't loaded. Never enable an integration whose mod isn't loaded.
