...
**2. Survey state silently.** Pull these without narrating each file load, and pull them in **two parallel batches** rather than one file per turn (see CLAUDE.md "Opening reads"): first the fixed paths — `clock.json`, `canon/INDEX.md`, `canon/city.md`, `canon/tone.md`, `canon/playthrough-premise.md`, `settings.json`, `carto/processed/index.md`, and the open `events/*.md` from step 1 — then a second batch for what `clock.json` pointed at plus the entity/district files the first batch showed to be relevant.
...
- The latest snapshot — the path in `clock.json`'s `latest_snapshot`, not a listing of `snapshots/` — required for grounding:
...
- **Current in-world date** — read `clock.json` at the city root (`in_world_date`). This is the live "now," refreshed every few seconds; use it as the event's open date and the base for computing the deadline — not the snapshot's `captured_at_ingame`, which can be in-world weeks stale. Fall back to the snapshot's `captured_at_ingame` only if `clock.json` is missing. (Same file carries `latest_snapshot`, so this read and the snapshot read above are one lookup, not two.)
...
**6b. Surface it as a chirp (if Custom Chirps is enabled).** If `"customchirps"` is in `settings.json.integrations`, write one short in-world chirp about this event to `chirp-requests.json` at the city root — the public version of the moment going live in the player's in-game Chirper feed. Follow CLAUDE.md "Chirping the city": terse (~200 chars), in the voice of whoever's driving the moment (`sender_name` = the pushing character, or a civic voice), a fitting `department` icon, and `event` set to this event's slug. One chirp, not one per option. Skip this step entirely if the integration is off. Don't mention the chirp to the player — the event narration in step 8 already tells them the moment; the chirp just makes it visible in-game.
