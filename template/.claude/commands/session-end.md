...
- **Elections (if loaded).** If `snapshot.politics` is present, the election cycle — candidate/party seeding, a new mayor, a concluded race, scandal pressure — was already handled by the `/events-resolve` "Election cycle" step in 0a. Here, just fold the political shift into the session log and `## Story consequences` (who took power, what it means for active arcs) and make sure implicated `characters/` / `factions/` / `secrets/` are consistent. Don't re-record events 0a already wrote.
