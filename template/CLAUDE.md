...
carto/        Spatial geography. Refreshed automatically on first export of a
  ...
  processed/    Pre-digested markdown — the part I actually read.
    ...
    roads.md       Named roads / highways / bridges with combined length and
                   per-segment count. Note: at t=0 most "named" roads are
                   map-generated by CS2, not player-laid — the storyteller
                   should treat highways as regional context (the road system
                   the city plugs into), not authorial choices. The first
                   road the player actually places is usually short.
    districts/
